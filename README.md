...
- default pipeline maintenance backfills now use backlog-specific routing: agenda segmentation runs heuristic-first with a shorter maintenance timeout, and summary hydration uses a shorter maintenance timeout plus deterministic fallback on provider failure.
