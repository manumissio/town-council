...
docker compose up -d inference api >/dev/null

echo "[bootstrap_models] Caching sentence-transformer model in shared volume..."
docker compose run --rm api python -c "from sentence_transformers import SentenceTransformer; SentenceTransformer('all-MiniLM-L6-v2')"

echo "[bootstrap_models] Downloading Gemma GGUF into shared volume..."
docker compose run --rm api python -c "from huggingface_hub import hf_hub_download; hf_hub_download(repo_id='unsloth/gemma-3-270m-it-GGUF', filename='gemma-3-270m-it-Q4_K_M.gguf', local_dir='/models')"
