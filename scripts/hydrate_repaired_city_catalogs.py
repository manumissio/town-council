...
@contextmanager
def _capture_summary_fallback_events() -> dict[str, int]:
    logger = logging.getLogger("local-ai")
    counts: Counter[str] = Counter()
...
def _looks_structured_enough_for_heuristic_segmentation(text: str | None) -> bool:
    value = text or ""
    if not value.strip():
        return False
    numbered = len(
...
class _HeuristicOnlyLocalAI:
    def extract_agenda(self, text: str) -> list[dict[str, Any]]:
        local_ai = llm_mod.LocalAI()
        original_get_provider = local_ai._get_provider
        try:
            # Maintenance mode uses the existing deterministic fallback parser directly
            # so repaired batches do not spend time timing out before landing in the same logic.
...
def _segment_one_catalog(catalog_id: int, *, segment_mode: str = "normal") -> dict[str, Any]:
    with db_session() as session:
        try:
            catalog = session.get(Catalog, catalog_id)
            if not catalog or not catalog.content:
                return {"status": "other", "llm_attempted": 0, "llm_skipped_heuristic_first": 0, "heuristic_complete": 0}
            doc = session.query(Document).filter_by(catalog_id=catalog.id).first()
            if not doc or not doc.event_id:
                return {"status": "other", "llm_attempted": 0, "llm_skipped_heuristic_first": 0, "heuristic_complete": 0}

            if segment_mode == "maintenance" and _looks_structured_enough_for_heuristic_segmentation(catalog.content):
                heuristic_resolved = resolve_agenda_items(session, catalog, doc, _HeuristicOnlyLocalAI())
                if heuristic_resolved.get("items"):
                    status = _persist_segmented_agenda(
                        session,
                        catalog=catalog,
                        doc=doc,
            ...
            status = _persist_segmented_agenda(session, catalog=catalog, doc=doc, items_data=resolved["items"])
            return {
                "status": status,
...
def _build_deterministic_agenda_summary_payload(catalog_id: int) -> dict[str, Any]:
    with db_session() as session:
        catalog = session.get(Catalog, catalog_id)
        if not catalog:
...
def _summarize_one_catalog(
    ...
    *,
    ...
    if summary_fallback_mode == "deterministic" and status == "error" and provider_issue:
        fallback_result = _build_deterministic_agenda_summary_payload(catalog_id)
