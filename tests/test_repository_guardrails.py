...
def _cleanup_inventory_names(node: ast.stmt) -> list[str]:
    ...
    return [
        ...
        and target.id.endswith("_CLEANUP_MODULES")
    ]
...
def _file_length_policy_nodes(module_path: Path) -> tuple[list[str], list[str]]:
    try:
        module_filename = str(module_path.relative_to(ROOT))
    except ValueError:
        module_filename = str(module_path)
    tree = ast.parse(
...
def test_file_length_policy_scan_detects_named_limits_without_flagging_reads(
    ...
    policy_module.write_text(
        "RENAMED_CLEANUP_MODULES = ('pipeline/example.py',)\n"
